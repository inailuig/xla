...
PYBIND11_MODULE(xla_extension, m) {
  ...
  m.def(
      "get_tfrt_cpu_client_mpi",
      [](bool asynchronous) -> std::shared_ptr<PyClient> {
        py::gil_scoped_release gil_release;
        std::unique_ptr<PjRtClient> client = xla::ValueOrThrow(GetTfrtCpuClientMPI(asynchronous));
        return std::make_shared<PyClient>(ifrt::PjRtClient::Create(std::move(client)));
      },
      py::arg("asynchronous") = true);
  m.def("mpi_finalize", [](){TfrtCpuClient::mpi_finalize();});
